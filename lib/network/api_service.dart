...
class ApiService {
  ...
  static Future<bool> _isAuthTokenValid() async {
    ...
    var lastRefreshTime = await SharedPreferenceLocalStorage.getLastRefreshTime();
  ...
  static Future<dynamic> putRequest({required endpoint, param: const {"": ""}, id}) async {
    try {
      print("--------------------");
      ...
      DioApi.Response response = await _dio.put(endpoint + "/" + id.toString(), data: param);
      return response;
    } on DioError catch (err) {
      await _checkForTokenAuthorizationError(err);
      return null;
  ...
  static Future<dynamic> deleteRequest({required endpoint, id}) async {
    try {
      ...
      DioApi.Response response = await _dio.delete(endpoint + "/" + id.toString());
      return response;
    } on DioError catch (err) {
      await _checkForTokenAuthorizationError(err);
      return null;
  ...
  static Future<dynamic> getRequest({endpoint, param: const {"": ""}}) async {
    try {
      ...
      dynamic response = await _dio.get(endpoint, queryParameters: param,);
      return response;
    } on DioError catch (err) {
      print(err);
      await _checkForTokenAuthorizationError(err);
  ...
  static Future<bool> getAuthToken() async {
    try {
      ...
      response = await dio.post(ApiEndpoint.HOST_URL + ApiEndpoint.AUTH_TOKEN, data: {
      ...
      await SharedPreferenceLocalStorage.setAccessToken(response.data["access_token"]);
      await SharedPreferenceLocalStorage.setAccessTokenType(response.data["token_type"]);
      await SharedPreferenceLocalStorage.setRefreshToken(response.data["refresh_token"]);
      await SharedPreferenceLocalStorage.setRefreshTime(response.data["expires_in"]);
      await SharedPreferenceLocalStorage.setLastRefreshTime(DateTime.now().hour);
      init();
  ...
  static Future<dynamic> registerUser({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.CREATE_USER, param: data);
  ...
  static Future<dynamic> verifyPhone({userId, param}) async {
    var response = await putRequest(endpoint: ApiEndpoint.VERIFY_PHONE, id: userId, param: param);
    return response;
  }

  static Future<dynamic> updatePhone({userId, param}) async {
    var response = await putRequest(endpoint: ApiEndpoint.UPDATE_PHONE, id: userId, param: param);
    return response;
  }

  static Future<dynamic> getUserDetails({firebaseId}) async {
    var response = await getRequest(endpoint: ApiEndpoint.GET_USER + '/' + firebaseId);
    return response;
  }

  static Future<dynamic> getCategory({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.MASTER_CATEGORY, param: data);
    return response;
  }
  ...
  static Future<dynamic> addPostData({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.CREATE_POST, param: data);
    return response;
  }

  static Future<dynamic> updatePostData({data, id}) async {
    var response = await putRequest(endpoint: ApiEndpoint.UPDATE_POST, param: data, id: id);
    return response;
  }

  static Future<dynamic> deletePostData({id}) async {
    var response = await deleteRequest(endpoint: ApiEndpoint.DELETE_POST, id: id);
    return response;
  }

  static Future<dynamic> getPostSearch({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.SEARCH_POST, param: data);
    return response;
  }
  ...
  static Future<dynamic> likePost({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.LIKE_POST, param: data);
    return response;
  }

  static Future<dynamic> isVerified({required String id}) async {
    var response = await postRequest(endpoint: ApiEndpoint.IS_VERIFIED + "/" + id);
    return response;
  }

  static Future<Response> getBooks() async {
    final response = await getRequest(
      endpoint: ApiEndpoint.getBook + '/',
    );
    return response;
  }

  static Future<dynamic> postBook({data}) async {
    var response = await postRequest(endpoint: ApiEndpoint.getBook, param: data);
    return response;
  }
}
